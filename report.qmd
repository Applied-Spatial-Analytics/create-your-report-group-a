...
cluster_grid <- function(gpkg_path, bc_col, patch_count_col, patch_cap_col, lcz_col,
  ...
  # --- Classify each raw kmeans cluster by its profile, not its arbitrary label ---
  ...
  # --- Remap to your fixed, city-specific ID and color scheme ---
...
# Rotterdam: matches your original QGIS numbering exactly
rt_role_to_id <- c(
  "Transition zones"        = 0,
  "Key corridors"            = 1,
  "Urban matrix"             = 2,
...
# Guangzhou: matches your original QGIS numbering exactly (different order!)
gz_role_to_id <- c(
  "Fragmented green"         = 0,
  "Urban matrix"             = 1,
  "Transition zones"         = 2,
...
![Morphological Spatial Pattern Analysis in Rotterdam](images/RT_mspa.png){#fig-MSPArt}
...
![Morphological Spatial Pattern Analysis in Guangzhou](images/GZ_mspa.png){#fig-MSPAgz}


#### Patches and Link Importance {#sec-result-ecoconnect-gz}

![Ecological connectivity within the Envelope.](images/FirstGZmap.png){#fig-connectivity-first width=100%}

![Ecological connectivity within the boundary of Guangzhou](images/Second_GZmap.png){#fig-connectivity-second width=100%}
...
### Zoom Ins Guangzhou
![Zoom in locations in Guangzhou](images/Third_GZmap.png){#fig-zoomin-gz width=100%}
...
![Ecological resistance analysis layers zoom in 1.](images/zoom1_metrics.png){#fig-zoom1-metrics width=100%}

![Ecological resistance analysis layers zoom in 2.](images/zoom2_metrics.png){#fig-zoom2-metrics width=100%}

##### Conclusion Maps

![Ecological resistance zoom in 1 (layers combined).](images/zoom1_resistance.png){#fig-resistance-first width=100%}

![Ecological resistance zoom in 2 (layers combined).](images/zoom2_resistance_final.png){#fig-resistance-second width=100%}
...
patches of subtropical forest. However, the highest-importance links visible as 
the darkest lines in the lower map cross several broader zones of moderate to 
high resistance. These zones are not narrow road lines but wider patches of urban 
fabric: built-up areas with buildings, paved surfaces and limited vegetation that 
interrupt the otherwise continuous forest matrix between the forested patches. 
Within these urban zones, red lines indicate that roads add an additional layer of 
very high resistance. The barrier to connectivity is therefore a combination of 
urban built fabric and the roads embedded within it.

Two complementary NbS are proposed to reduce resistance across these urban zones 
along the critical link trajectories.

First, for the roads within the urban fabric, vegetated wildlife underpasses are 
proposed at the specific locations where the highest-importance links cross them. 
Cork et al. (2024) demonstrate that native vegetation planted directly at and 
around crossing structures significantly increases their use by wildlife, and that 
verge vegetation quality and continuity guide species movement toward crossing 
points. Given the subtropical forest context, underpasses should connect directly 
to existing forest edge vegetation on both sides.
...
permeability of the urban matrix along the link corridors (@fig-greenroofs). Madre 
et al. (2018) show that green roofs and green walls can function as stepping stones 
in fragmented urban areas, enhancing species dispersal through the urban matrix. 
This is further supported by Joshi & Teller (2021), whose systematic review of 
green roof integration in urban areas explicitly identifies green roofs as a medium 
for connecting various urban green areas, thereby providing the potential for 
habitat connectivity and maintenance of the ecological network. Importantly, Joshi 
& Teller (2021) also point out that the height of buildings is a determining factor 
for biodiversity effectiveness of green roofs, and call for future research to 
include taller buildings in ecological network analyses a relevant consideration 
given the high-rise character of parts of the Tianhe urban fabric visible in the 
resistance map. Green roofs should therefore be prioritised on lower-rise buildings 
along the link trajectory, where their connectivity potential is greatest.

![Schematic representing the diverse processes involved in conventional roofs and 
green roofs (Joshi & Teller, 2021).](images/greenroofs.png){#fig-greenroofs width=100%}

Together these two interventions address both barrier types identified in the 
resistance map: the roads through targeted crossing structures, and the surrounding 
urban matrix through a network of distributed green stepping stones following the 
trajectory of the highest-importance links.
